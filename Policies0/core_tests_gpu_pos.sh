...
PARAM_LINE=$(sed -n "${TASK_ID}p" params_pos.csv)
#PARAM_LINE=$(sed -n "$((SLURM_ARRAY_TASK_ID))p" params2.csv)
IFS=',' read -r THRESH_POS ACTION_TYPE SEED<<< "$PARAM_LINE  "

# Run the script
srun --export=ALL python td3_v0.py --threshold_pos $THRESH_POS --threshold_ori 0 --action_type $ACTION_TYPE --seed $SEED --ran $TASK_ID
