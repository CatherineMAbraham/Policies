...
def next_thresholds(threshold_pos, threshold_ori):
    threshold_pos = max(threshold_pos -0.00005, 0)
    threshold_ori = max(threshold_ori - np.deg2rad(0.05), 0)
    
    return threshold_pos, threshold_ori
def train(threshold_pos=0.001, threshold_ori=np.deg2rad(6), action_type='pos_only',seed=42,ran="1"):
    ...
    for i in range(10):
        ...
        for e in env.envs:
            e.unwrapped.distance_threshold_pos = current_threshold_pos
            e.unwrapped.distance_threshold_ori = current_threshold_ori

        eval_env.set_attr('distance_threshold_pos', current_threshold_pos)
    ...
    ## Stop training callback based on success rate, model_save_path None and just setting it to save any best model in eval 
        ...
        model.learn(1_500_000, callback=eval_callback,reset_num_timesteps=False)
...
if __name__ == "__main__":
    ...
    threshold_pos, threshold_ori = train(threshold_pos=args.threshold_pos, threshold_ori=args.threshold_ori, action_type=args.action_type, seed = args.seed, ran=args.ran)